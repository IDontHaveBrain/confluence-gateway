...
def check_test_collection() -> bool:
    ...
    try:
        result = subprocess.run(["uv", "run", "pytest", "tests/", "--collect-only", "-q"], 
                              capture_output=True, text=True)
        if result.returncode == 0:
...
def main():
    ...
    required_env_ok = all(is_set for var, is_set, req_type in env_results if req_type == "required")
    print(f"Environment Variables: {'✅ READY' if required_env_ok else '❌ MISSING REQUIRED'}")
