...
class RealDataGenerator:
    ...
    def get_content_by_category(self, category: str, limit: int) -> List[Dict[str, Any]]:
        entries = []
    ...
    def create_space(self, space_config: Dict[str, Any], dry_run: bool = False) -> Optional[str]:
        prefix = self.production_config['generation']['prefix']
        space_key = f"{prefix}{space_config['key']}"
        space_name = f"{prefix} - {space_config['name']}"
        
        if dry_run:
            console.print(f"[yellow]DRY RUN: Would create space {space_key} - {space_name}[/yellow]")
            return space_key
    ...
    def format_content_for_confluence(self, entry: Dict[str, Any]) -> str:
        return self.formatter.format_entry_for_confluence(entry)
        
    def create_pages_in_space(self, space_key: str, space_config: Dict[str, Any], dry_run: bool = False):
        pages_count = space_config['pages_count']
        if os.environ.get('TEST_MODE') == '1':
            pages_count = min(3, pages_count)
        all_entries = []
        for category in space_config['categories']:
            entries = self.get_content_by_category(category, pages_count // len(space_config['categories']) + 5)
        ...
        for entry in track(all_entries, description=f"Creating pages in {space_key}"):
            title = f"{self.production_config['generation']['prefix']} - {entry['title']}"
    ...
    def generate_all(self, dry_run: bool = False, recreate: bool = False):
        ...
        console.print("[bold blue]Creating Confluence Test Data from Real Documentation[/bold blue]\n")
    ...
    def cleanup(self, pattern: Optional[str] = None):
        ...
        if not self.created_spaces and not self.created_pages:
            console.print(f"[yellow]No tracked data found. Searching for spaces with pattern '{pattern}'...[/yellow]")
        ...
        for page_info in self.created_pages[:]:
            try:
                ...
                console.print(f"[yellow]Could not delete page {page_info['title']}: {e}[/yellow]")
        deleted_spaces = 0
        for space_info in self.created_spaces[:]:
            try:
                space_key = space_info['key']
                console.print(f"[yellow]Attempting to delete space {space_key}...[/yellow]")
                ...
                try:
                    ...
                    try:
                        ...
                        console.print(f"[yellow]Could not delete or archive space {space_key}: {delete_error}[/yellow]")
                ...
                try:
                    ...
                    while True:
                        try:
                            ...
                            elif hasattr(pages_response, 'get') and 'results' in pages_response:
