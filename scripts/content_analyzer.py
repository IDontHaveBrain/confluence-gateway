...
class ContentAnalyzer:
    def __init__(self):
        self.api_patterns = {
            'endpoint': re.compile(r'(GET|POST|PUT|DELETE|PATCH)\s+(/[^\s]+)', re.IGNORECASE),
    ...
    def analyze_content(self, content: str, source_info: Dict[str, Any]) -> StructuredContent:
    ...
    def _classify_content_type(self, content: str, source_info: Dict[str, Any]) -> ContentType:
        content_lower = content.lower()
        
        # Check for API documentation patterns
        if (self.api_patterns['endpoint'].search(content) or
            'api' in source_info.get('name', '').lower() or
            any(word in content_lower for word in ['endpoint', 'parameters', 'response', 'request'])):
            return ContentType.API_DOCUMENTATION
        
        # Check for tutorial patterns
        if any(word in content_lower for word in ['tutorial', 'getting started', 'quickstart', 'guide']):
            return ContentType.TUTORIAL
        
        # Check for troubleshooting patterns
        if any(word in content_lower for word in ['error', 'troubleshoot', 'problem', 'issue', 'fix']):
            return ContentType.TROUBLESHOOTING
        
        # Check for reference patterns
        if any(word in content_lower for word in ['reference', 'documentation', 'spec', 'manual']):
    ...
    def _get_context_around_match(self, content: str, match, context_length: int) -> str:
        start = max(0, match.start() - context_length)
        end = min(len(content), match.end() + context_length)
        return content[start:end].strip()

def analyze_collected_data(index_file: str) -> Dict[str, Any]:
    ...
    for entry in data['entries']:
        structured = analyzer.analyze_content(entry['content'], {
            'title': entry['title'],
        ...
        analysis_results['content_types'][content_type] = analysis_results['content_types'].get(content_type, 0) + 1
        
        analysis_results['structure_stats']['total_headers'] += len(structured.headers)
        analysis_results['structure_stats']['total_code_blocks'] += len(structured.code_blocks)
        analysis_results['structure_stats']['total_tables'] += len(structured.tables)
        analysis_results['structure_stats']['total_api_endpoints'] += len(structured.api_endpoints)
        
        for code_block in structured.code_blocks:
            analysis_results['structure_stats']['languages_found'].add(code_block['language'])
    ...
    analysis_results['structure_stats']['languages_found'] = list(analysis_results['structure_stats']['languages_found'])
